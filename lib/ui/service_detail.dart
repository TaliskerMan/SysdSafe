...
class _ServiceDetailScreenState extends State<ServiceDetailScreen> {
  ...
  Future<void> _applyAutoFix(List<HardeningAdvice> tier1Advice) async {
    ...
    if (serviceName.contains('/') || serviceName.contains('..')) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error: Invalid service name format.')));
      return;
    }
    ...
    try {
      ...
      final catResult = await Process.run('systemctl', ['cat', '--', serviceName]);
      if (catResult.exitCode == 0) {
        ...
        await DatabaseHelper.instance.backupServiceState(serviceName, originalContent);
        ...
        throw Exception('Systemctl cat failed. Aborting auto-fix to ensure system safety.');
      }
    } catch (e) {
      LogService.error('Backup failed for $serviceName: $e');
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Auto-Fix aborted due to backup failure: $e')));
    ...
    final command = 'mkdir -p "\$1" && printf "%b\\n" "\$2" > "\$3" && systemctl daemon-reload && systemctl try-restart -- "\$4"';
    
    try {
      final result = await Process.run('pkexec', [
        'sh', '-c', command, '--', dirPath, fileContent, filePath, serviceName
      ]);
      if (result.exitCode == 0) {
        LogService.info('Auto-Fix applied successfully for $serviceName');
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Auto-Fix applied successfully! Backups saved.')));
      } else {
        LogService.error('Auto-Fix failed for $serviceName: ${result.stderr}');
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed: ${result.stderr}')));
      }
    } catch (e) {
  ...
  Future<void> _revertAutoFix() async {
    ...
    final command = 'rm -f -- "\$1" && systemctl daemon-reload && systemctl try-restart -- "\$2"';
    
    setState(() => isLoading = true);
    try {
      final result = await Process.run('pkexec', [
        'sh', '-c', command, '--', filePath, serviceName
      ]);
      if (result.exitCode == 0) {
        LogService.info('Auto-Fix reverted successfully for $serviceName');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Auto-Fix reverted! Original config available in ~/sysdsafe_backups/'),
            duration: Duration(seconds: 4),
          ));
        }
      } else {
        LogService.error('Revert failed for $serviceName: ${result.stderr}');
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Revert failed: ${result.stderr}')));
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    final bool isDangerousService = widget.service.name.startsWith('user@') || widget.service.name.contains('greeter');
              ...
              child: CustomScrollView(
                slivers: [
                      ...
                      child: Card(
                        ...
                        child: Padding(
                          ...
                          child: Row(
                            ...
                            children: [
                              Icon(Icons.warning_amber_rounded, color: isDark ? Colors.white : Colors.amber[900]),
                              ...
                              Expanded(
                                child: Text(
                                  ...
                                  style: TextStyle(fontSize: appState.fontSizeBase, color: isDark ? Colors.white : Colors.amber[900]),
                                ),
                              ),
                            ],
                          ),
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                Row(
                                  children: [
                                    Icon(Icons.warning, color: isDark ? Colors.white : Colors.red[900]),
                                    const SizedBox(width: 8),
                                    Text(
                                      'CRITICAL WARNING',
                                      style: TextStyle(fontSize: appState.fontSizeBase + 4, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.red[900]),
                                    ),
                                  ],
                                ),
                                ...
                                Text(
                                  ...
                                  style: TextStyle(fontSize: appState.fontSizeBase, color: isDark ? Colors.white : Colors.red[900]),
                                ),
                              ],
                            ),
                          ),
                            ...
                            child: Column(
                              ...
                              children: [
                                Text(
                                  ...
                                  style: TextStyle(fontSize: appState.fontSizeBase + 4, fontWeight: FontWeight.bold),
                                ),
                                const SizedBox(height: 8),
                                Text(
                                  '1. Open terminal.\n'
                  ...
                  if (tieredAdvice[1]!.isNotEmpty) ...[
                    isDangerousService 
                      ? _buildTierHeader('Tier 1: Quick Wins (Low Risk) - AUTO-FIX DISABLED', Colors.green, appState)
                    ...
                    SliverList(
                      delegate: SliverChildBuilderDelegate(
                        (context, index) => _buildAdviceCard(tieredAdvice[1]![index], appState, isDark),
                  ...
                  if (tieredAdvice[2]!.isNotEmpty) ...[
                    const SliverToBoxAdapter(child: SizedBox(height: 16)),
                    _buildTierHeader('Tier 2: Contextual Hardening (Medium Risk)', Colors.orange, appState),
                    SliverList(
                      delegate: SliverChildBuilderDelegate(
                        (context, index) => _buildAdviceCard(tieredAdvice[2]![index], appState, isDark),
                  ...
                  if (tieredAdvice[3]!.isNotEmpty) ...[
                    const SliverToBoxAdapter(child: SizedBox(height: 16)),
                    _buildTierHeader('Tier 3: Advanced Isolation (High Risk)', Colors.redAccent, appState),
                    SliverList(
                      delegate: SliverChildBuilderDelegate(
                        (context, index) => _buildAdviceCard(tieredAdvice[3]![index], appState, isDark),
  ...
  Widget _buildTierHeader(String title, Color color, AppState appState) {
    return SliverToBoxAdapter(
      child: Padding(
        ...
        child: Text(
          title,
          style: TextStyle(fontSize: appState.fontSizeBase + 4, fontWeight: FontWeight.bold, color: color),
        ),
      ),
    );
  }

  Widget _buildTierHeaderWithActions(String title, Color color, AppState appState, {required VoidCallback onAutoFix, required VoidCallback onRevert}) {
    return SliverToBoxAdapter(
      child: Padding(
        padding: const EdgeInsets.only(bottom: 8.0, top: 16.0),
        child: Row(
          ...
          children: [
            ...
            ElevatedButton.icon(
              ...
              label: Text('Auto-Fix', style: TextStyle(fontSize: appState.fontSizeBase)),
            ...
            TextButton.icon(
              ...
              label: Text('Revert', style: TextStyle(fontSize: appState.fontSizeBase)),
  ...
  Widget _buildAdviceCard(HardeningAdvice advice, AppState appState, bool isDark) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              advice.humanAdvice,
              style: TextStyle(fontSize: appState.fontSizeBase, color: isDark ? Colors.grey[400] : Colors.grey[700]),
            ),
            ...
            Container(
              ...
              child: Row(
                children: [
                  Icon(Icons.code, size: 16, color: isDark ? Colors.greenAccent : Colors.green),
                  const SizedBox(width: 8),
                  Text(
                    advice.snippet,
                    style: TextStyle(
